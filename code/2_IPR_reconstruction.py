...
# IPR reconstruction
rec_field = IPR(W,H,wavelength,z2,pitch_size,numPixels,hologram_amplitude,50)
...
def normal(x):
    ...
    x = x - x.min()
    return x / (x.max() + eps)
ssim_amp = ssim(normal(am_field_after_object), normal(am_rec), data_range=1.0)
print("SSIM (amplitude) =", ssim_amp)
